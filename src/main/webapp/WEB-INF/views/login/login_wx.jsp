					...
					<div class="panel-body">
						<div class="box-body">
							<form class="form-horizontal" id="loginform" role="form"
								method="post" action="${ctx}/login/loginValidateWx">
									...
									<select id="e1" class="col-xs-12" name="customerId" id="customerId"
										...
										<c:if test="${!empty etplist}">
											<c:forEach items="${etplist}" var="etp" varStatus="i">
												<option value="${etp.customerId}">${etp.accountName}</option>
											</c:forEach>
										</c:if>
									</select>
								</div>
								<div class="form-group">
									<input class="form-control" placeholder="用户帐号" name="account"
										type="text" autofocus>
								</div>
								<div class="form-group">
									<input class="form-control" placeholder="用户密码" name="password"
										type="password" value="">
								</div>
								<input type="submit" id="btn_login"
									class="btn btn-lg btn-success btn-block" value="验证">
							</form>
						</div>
					</div>
				</div>
